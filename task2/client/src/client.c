...
struct client* create_client(const char* ip, const int port) {
  ...
  client->serv_endpoint = atoe(&client->serv);
  ...
  client->service_fd = socket(AF_INET, SOCK_STREAM, 0);
  if (client->service_fd == -1)
    print_error("socket");
...
void run_client(struct client* client) {
  ...
  /* Receive new endpoint */
  char* message = recv_message(client, client->sfd);
  if (message == NULL) {
    print_error("recv_message");
  }
  else if (strcmp(message, "occupied") == 0) {
    printf("Server is occupied\n");
    return;
  }
  ...
  /* Parse message */
  struct endpoint* endpoint = stoe(message);
  
  /* Change address of server */
  client->serv_endpoint = endpoint;
  client->serv.sin_addr.s_addr = inet_addr(endpoint->ip);
  client->serv.sin_port = htons(endpoint->port);
  
  /* Conenct to server */
  serv_size = sizeof(client->serv);
  if (connect(client->service_fd, (struct sockaddr*) &client->serv, serv_size) == -1)
    print_error("connect");
...
void process_input(struct client* client) {
  ...
  while (1) {
    ...
    /* Send user message */
    send_message(client, client->service_fd, buffer);

    /* Receive answer */
    char* message = recv_message(client, client->service_fd);
    if (message == NULL) {
      close_connection(client);
      break;
    }
 ...
 */
void send_message(struct client* client, int fd, const char* buffer) {
 ...
 */
char* recv_message(struct client* client, int fd) {
  uint32_t net_len;
  ...
  bytes_read = recv(fd, &net_len, sizeof(net_len), 0);
  /* Error occured*/
  if (bytes_read < 0) {
    print_error("recv");
  } 
  ...
  while (total_received < message_len) {
    bytes_read = recv(fd, message + total_received, message_len - total_received, 0);
    if (bytes_read < 0) {
      free(message);
      print_error("recv");
    }
